...
class AverageRatings(Resource):
    ...
    def get(self):
        ...
        data = [
            {"restaurant_id": r.restaurant_id, "average_rating": float(r.avg_rating)}
            for r in results
        ]
        return data, 200
...
class Top3Restaurants(Resource):
    ...
    def get(self, cuisine_type):
        """
        Returns top 3 restaurants by cuisine type, sorted by average rating (desc).
        [
          {
            "restaurant_id": 1,
            "name": "ABC",
        ...
        results = (
            db.session.query(Restaurant.id, Restaurant.name, Restaurant.cuisine_type, subq.c.avg_rating)
